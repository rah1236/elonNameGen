...
class InteractionLayer : Layer {
    ...
    init() {
        ...
        let independentButton = Button(name:"Alone", labelString:"Generate Elon Name", topLeft:Point(x:50, y:50))
    ...
    func genElonName()->String{
        let letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        let firstLetter = String((0..<1).map{ _ in letters.randomElement()! })
        let thirdLetter = String((0..<1).map{ _ in letters.randomElement()! })

        let ligatures = "ÆŒꜨẞꝎﬆꭁ🙰ꜼꜺ"
        let secondLetter = String((0..<1).map{ _ in ligatures.randomElement()! })

        let number = Int.random(in: 1..<99)
        return "\(firstLetter) \(secondLetter) \(thirdLetter)-\(number)"
    }
    ...
    func onButtonClickHandler(control:Control, localLocation:Point) {
        if let button = control as? Button {
            button.labelString = genElonName()
        }
    }
